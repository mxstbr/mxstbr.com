...
Parents talk to ChatGPT using `chores2_*` MCP tools or `pnpm chores2`. Telegram delivers notifications. Dedicated parent screens, PIN/admin workflows, Clippy, conversational Telegram commands, and Telegram approval/undo buttons are **not current behavior**. These exclusions apply even where a parent capability (such as editing a chore) is retained through tools.

The current data is independent in `chores:mxstbr:v2`, using existing infrastructure. The old board is `/chores2`; `pnpm chores` and unprefixed MCP tools affect only that separate legacy data. Catalog/history were copied once for the trial; only star balances were reconciled at promotion. Continuing synchronization or silently sharing mutable state with the legacy board is **not current behavior**.
