...
The [September 23 webhook audit](mcp-events-conformance.md) pins the current merged draft and tracks pending discovery changes separately. Missing subscription keys return NotFound, quotas return ResourceExhausted, and integer TTL suggestions clamp to the grant range. `maxAgeMs` bounds the entire initial replay even when notification timestamps and commit order differ.
