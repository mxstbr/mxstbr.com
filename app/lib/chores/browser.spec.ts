...
test.describe('overnight blackout', () => {
  ...
  test('starts at 8:30pm Pacific when idle, still wakes on tap, and clears at 6am', async ({
    ...
    await sleep.click()
    await expect(sleep).toHaveCount(0)
    await expect(page.locator('html')).not.toHaveClass(/chores-asleep/)
    await expect(page.locator('meta[name="theme-color"]')).toHaveAttribute(
      'content',
      '#fff8e8',
    )
...
test('landscape iPad: focus, stars, exact undo, rewards, packing, color, summary, retries and expiry', async ({
  ...
  await dilan.locator('.chores-period-reward').click()
  await expect(
  ...
  const balance = Number(
    (await devina.locator('.chores-wallet').innerText()).split(' ')[0],
  )
  const siblingWallet = await dilan.locator('.chores-wallet').innerText()
  await expect(
    devina.getByRole('heading', { name: 'Make your bed' }),
  ).toBeVisible()
  await expect(devina.locator('.chores-task-emoji')).toHaveText('🛏️')
  ...
  await expect(devina.locator('.chores-wallet')).toHaveText(`${balance + 4} ★`)
  await expect(devina.getByText('Bonus earned!')).toBeVisible()
  await expect(dilan.locator('.chores-wallet')).toHaveText(siblingWallet)
  await devina.getByRole('button', { name: /^Done/ }).click()
  ...
  await devina
    ...
    .first()
    .click()
  await expect(devina.locator('.chores-wallet')).toHaveText(`${balance + 1} ★`)
  ...
  await expect(devina.locator('.chores-wallet')).toHaveText(`${balance - 16} ★`)
  await expect(
    devina.getByRole('heading', { name: 'Rewards', exact: true }),
  ).toBeVisible()
  await expect(dilan.locator('.chores-wallet')).toHaveText(siblingWallet)
  ...
  await devina.locator('.chores-name').click()
  ...
  await devina.getByRole('button', { name: 'Save my color' }).click()
  await devina.locator('.chores-period-reward').click()
  ...
  await expect(devina.locator('.chores-wallet')).toHaveText(`${balance - 15} ★`)
  expect(ids).toHaveLength(2)
  expect(ids[0]).toBe(ids[1])
  await page.unroute('**/api/chores/command')
  await page.emulateMedia({ colorScheme: 'dark', reducedMotion: 'reduce' })
  await page.screenshot({ path: '/private/tmp/chores-dark.png' })
  await page.emulateMedia({ colorScheme: 'light' })
  await page.screenshot({ path: '/private/tmp/chores-verified.png' })
...
test('a glance separates outstanding chores from empty, finished and pending work without a false all-clear', async ({
  ...
  await expect(darian.locator('.chores-primary')).toHaveCount(0)
  ...
  await page.route('**/api/chores/command', (route) =>
    route.fulfill({
      status: 503,
