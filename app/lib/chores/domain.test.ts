...
import { PRE_RENAME_SOURCE, CLI_ACTOR_ID } from './compatibility'
...
test('pre-rename on-time submissions remain approvable and exactly reversible', async () => {
  const t = setup()
  const occurrence = t.repo.days['2026-09-09'].occurrences.find(
    (o) => o.kidId === 'kid-3' && o.choreId.endsWith('-bed'),
  )!
  occurrence.chore.requiresApproval = true
  const submitted = await t.run({
    action: 'submit',
    occurrenceId: occurrence.id,
  })
  const saved = t.repo.days['2026-09-09'].submissions.find(
    (s) => s.id === submitted.id,
  )!
  ...
  assert.equal(reviewed.balance, 27)
  await t.run({ action: 'undo', submissionId: saved.id }, parent)
  assert.equal(t.repo.core.balances['kid-3'], 26)
  assert.equal(
    (await t.run({ action: 'undo', submissionId: saved.id }, parent)).status,
    'already_processed',
  )
})

test('renamed CLI preserves the receipt identity of an uncertain earlier command', async () => {
  const t = setup()
  const command = {
    action: 'adjust_stars',
    kidId: 'kid-3',
    delta: 5,
    note: 'Independent adjustment',
  }
  const requestId = 'pre-rename-uncertain-command'
  const previous = await t.run(
    command,
    { kind: 'parent', id: 'chores2-cli' },
    requestId,
  )
  assert.deepEqual(
    await t.run(command, { kind: 'parent', id: CLI_ACTOR_ID }, requestId),
    previous,
  )
  assert.equal(t.repo.core.balances['kid-3'], 31)
})
