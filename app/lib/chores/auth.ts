...
export function parentActor(): Actor {
  if (!parentContext.getStore())
    fail(
      'FORBIDDEN',
      'Chores parent tools require an authenticated MCP connection. A kid device session is not sufficient.',
    )
...
export async function unlockWithSitePassword(password: string) {
  if (!sitePasswordMatches(password))
    fail('FORBIDDEN', 'That password isn’t right. Use the site password.')
...
export async function deviceActor(): Promise<Actor | null> {
  ...
  for (const name of [COOKIE, PRE_RENAME_COOKIE]) {
    const token = cookieStore.get(name)?.value
    if (!token || !/^[a-f0-9]{64}$/.test(token)) continue
...
export async function inviteDevice(
  ...
  return {
    ...
    expiresInHours: 24,
    message:
      'Open this one-use link on the shared iPad. It grants access only to the Chores kid board.',
  }
}
