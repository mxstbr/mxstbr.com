...
export const meta = {
  ...
  title: 'Why dev tools are hard startups',
  summary: 'TODO',
  views: 0,
}
...
But how does a blacksmith go from making a better hammer to distributing that hammer to lots of other people? That requires a ton of work and a skill set (building a product business) that they don’t need as part of their usual work.
...
Finally, we are an ecosystem of inventors that can share new ideas with everybody else in a second. Thus unsurprisingly, new frameworks, libraries, and packages are invented that change the way we work (for the better!) all the time.
